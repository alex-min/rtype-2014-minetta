...
void WindowsNetworkManager::generateReadFs()
{
         ...
         it != _network.end(); ++it)
    {
        if ((*it)->getSocket()->WindowsGetSocket() != SOCKET_ERROR)
         FD_SET((*it)->getSocket()->WindowsGetSocket(), &_readfs);
    }
}
...
void WindowsNetworkManager::generateWriteFs()
{
         ...
         ; ++it)
    {
        if (!((*it)->getWriteBuffer()->isEmpty()))
        {
            ...
            {
                FD_SET((*it)->getRemoteNetwork()->getSocket()->WindowsGetSocket(), &_writefs);
            }
            else if ((*it)->getSocket()->WindowsGetSocket() != SOCKET_ERROR)
                FD_SET((*it)->getSocket()->WindowsGetSocket(), &_writefs);
        }
    }
}
...
void WindowsNetworkManager::udpClientWriteEvent(Network::Network *net, Network::Network *server)
{
...
void WindowsNetworkManager::udpServerReadEvent(Network::Network *net)
{
    ...
    try {
        UInt16 byteRead = net->getSocket()->readFrom(&_udpReceivedIp, &_udpReceivedPort,
         ...
         Network::Network *net2 = WindowsNetworkManager::getUdpClient(net);

         //LOG << " ReadEvent (" << byteRead << " bytes read)" << std::endl;
        net2->getReadBuffer()->append(_mainBuffer, byteRead);
        if (_slot) _slot->onRead(net, net2);
    } catch (NetworkDisconnect &) {
        if (_slot) _slot->onDisconnect(net);
        WindowsNetworkManager::removeNetwork(net);
        throw;
...
void WindowsNetworkManager::udpClientReadEvent(Network::Network *net)
{
    try {
        UInt16 byteRead = net->getSocket()->readFrom(&_udpReceivedIp, &_udpReceivedPort,
                                   _mainBuffer,
    ...
    if (net->getSocket()->isServerSock())
    {
        if (net->getSocket()->getType() == Network::ISocket::TCP) {
         ...
         if (net->getSocket()->getType() == Network::ISocket::TCP) {
             ...
             WindowsNetworkManager::udpClientReadEvent(net);
            // Cannot exist
         }
    }
}
                 ...
                 it != _network.end(); ++it) {
                    if (WindowsNetworkManager::isAClientFrom(*it, net)) {
                        if ((*it)->getWriteBuffer()->getReadSize() > 0) {
                            WindowsNetworkManager::udpClientWriteEvent(*it, net);
                        }
                    }
            }
        } else {
            if (net->getWriteBuffer()->getReadSize() > 0)
                WindowsNetworkManager::udpClientWriteEvent(net, net);
    ...
    while (1)
    {
             ...
             ; ++it)
        {
            if ((*it)->getSocket() && (*it)->getSocket()->WindowsGetSocket() != SOCKET_ERROR &&
                    FD_ISSET((*it)->getSocket()->WindowsGetSocket(), &_readfs))
            {
                WindowsNetworkManager::readEvent(*it);
                FD_CLR((*it)->getSocket()->WindowsGetSocket(), &_readfs);
                break;
            }
            else if ((*it)->getSocket() && (*it)->getSocket()->WindowsGetSocket() != SOCKET_ERROR
                     && FD_ISSET((*it)->getSocket()->WindowsGetSocket(), &_writefs))
            {

...
void UNIXNetworkManager::udpClientWriteEvent(Network::Network *net, Network::Network *server)
{
...
void UNIXNetworkManager::udpServerReadEvent(Network::Network *net)
{
    ...
    try {
        UInt16 byteRead = net->getSocket()->readFrom(&_udpReceivedIp, &_udpReceivedPort,
         ...
         Network::Network *net2 = UNIXNetworkManager::getUdpClient(net);
...
void UNIXNetworkManager::udpClientReadEvent(Network::Network *net)
{
    try {
        UInt16 byteRead = net->getSocket()->readFrom(&_udpReceivedIp, &_udpReceivedPort,
                                   _mainBuffer,
    ...
    if (net->getSocket()->isServerSock())
    {
        if (net->getSocket()->getType() == Network::ISocket::TCP) {
         ...
         if (net->getSocket()->getType() == Network::ISocket::TCP) {
             ...
             UNIXNetworkManager::udpClientReadEvent(net);
            // Cannot exist
         }
    }
}
                 ...
                 it != _network.end(); ++it) {
                    if (UNIXNetworkManager::isAClientFrom(*it, net)) {
                        if ((*it)->getWriteBuffer()->getReadSize() > 0) {
                            UNIXNetworkManager::udpClientWriteEvent(*it, net);
                        }
                    }
            }
        } else {
            if (net->getWriteBuffer()->getReadSize() > 0)
                UNIXNetworkManager::udpClientWriteEvent(net, net);
